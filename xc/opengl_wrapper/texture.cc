...
void Texture::set_parameter(TextureWrap wrap_s, TextureWrap wrap_t,
     ...
     glTexParameteri(enum_to_gl(target_),  GL_TEXTURE_WRAP_S,
                        enum_to_gl(wrap_s));
     glTexParameteri(enum_to_gl(target_),  GL_TEXTURE_WRAP_T,
                        enum_to_gl(wrap_t));
...
void Texture::set_image_2d(int32_t level, TextureFormat internal_format,
     ...
     glTexImage2D(enum_to_gl(target_), level, enum_to_gl(internal_format),
                     width, height, 0, enum_to_gl(format),  GL_UNSIGNED_BYTE,
                     data);
}
